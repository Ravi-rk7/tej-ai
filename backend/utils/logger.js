...
const SENSITIVE_KEY = /(authorization|cookie|token|secret|password|email|image|buffer|body|payload|raw|url|user.?id|scan.?id)/i;
...
const sanitizeValue = (value, key = '') => {
    ...
    if (typeof value === 'string') {
        if (EMAIL_VALUE.test(value) || URL_VALUE.test(value) || BEARER_VALUE.test(value)) {
            return '[redacted]';
        }
