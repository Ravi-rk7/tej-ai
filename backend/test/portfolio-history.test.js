...
test('normalizes a routine fallback 169', () => {
  assert.equal(normalizeStoredRoutine({ morning: [{ title: 'Cleanser' }], night: [] }).morning[0].name, 'Cleanser');
});
test('maps a glasses warning 170', () => {
  assert.equal(buildQualityWarnings({ imageQuality: { glasses: true } })[0].code, 'GLASSES_DETECTED');
});
test('round-trips a history cursor 171', () => {
  const value = { createdAt: '2026-07-15T12:00:00.000Z', scanId: '00000000-0000-0000-0000-000000000008' };
  assert.deepEqual(decodeHistoryCursor(encodeHistoryCursor(value)), value);
});
...
test('accepts a metric at its maximum 173', () => {
  const metric = { key: 'glow', label: 'Glow', value: 100, min: 0, max: 100, unit: 'points', direction: 'higher', definition: 'glow-v1' };
  assert.equal(validMetric(metric), 'glow-v1');
});
test('normalizes sample warnings 174', () => {
  const result = normalizePortfolioResult({ schemaVersion: 2, scanId: 'sample-174', createdAt: '2026-07-15T12:00:00.000Z', source: 'sample', provider: { name: 'facepp', mappingVersion: 'categories-v1' }, metrics: [] });
  assert.equal(result.warnings.length, 1);
});
test('keeps a progress baseline null 175', () => {
  const metric = { key: 'glow', label: 'Glow', value: 70, min: 0, max: 100, unit: 'points', direction: 'higher', definition: 'glow-v1' };
  const result = buildPortfolioProgress([{ scanId: 'one', createdAt: '2026-07-15T12:00:00.000Z', provider: { name: 'facepp', mappingVersion: 'categories-v1' }, overallScore: metric, metrics: [] }]);
  assert.equal(result[0].baselineDelta, null);
});
test('serializes a null skin type 176', () => {
  const result = serializeScanResult({ id: 'scan-176', created_at: '2026-07-15T12:00:00.000Z', provider: 'ailabtools', provider_version: 'v1', glow_score: 70, skin_type: null, concerns: [], routine: [], metrics: {} });
  assert.equal(result.skinType, null);
});
...
test('accepts a regional timezone 179', () => {
  assert.equal(validateTimezone('Africa/Nairobi'), true);
});
test('rejects an unknown timezone 180', () => {
  assert.equal(validateTimezone('Mars/Colony'), false);
});
test('finds a longest run after a gap 181', () => {
  assert.equal(calculateLongestStreak(['2026-07-01', '2026-07-02', '2026-07-04'], '2026-07-04'), 2);
});
test('keeps a new streak inactive 182', () => {
  assert.equal(calculateStreak([], '2026-07-15').state, 'inactive');
});
test('calculates an empty adherence window 183', () => {
  assert.equal(calculateAdherence({ checkins: [], today: '2026-07-15', trackingStartedOn: '2026-07-15', days: 1 }).percentage, 0);
});
test('builds a future heatmap cell 184', () => {
  const map = buildHeatmap({ today: '2026-07-15', trackingStartedOn: '2026-07-15', weeks: 13 });
  assert.equal(map.days.at(-1).level, 0);
});
...
test('round-trips a history cursor 187', () => {
  const value = { createdAt: '2026-07-15T12:00:00.000Z', scanId: '00000000-0000-0000-0000-000000000008' };
  assert.deepEqual(decodeHistoryCursor(encodeHistoryCursor(value)), value);
});
...
test('normalizes sample warnings 190', () => {
  const result = normalizePortfolioResult({ schemaVersion: 2, scanId: 'sample-190', createdAt: '2026-07-15T12:00:00.000Z', source: 'sample', provider: { name: 'facepp', mappingVersion: 'categories-v1' }, metrics: [] });
  assert.equal(result.warnings.length, 1);
});
test('keeps a progress baseline null 191', () => {
  const metric = { key: 'glow', label: 'Glow', value: 70, min: 0, max: 100, unit: 'points', direction: 'higher', definition: 'glow-v1' };
  const result = buildPortfolioProgress([{ scanId: 'one', createdAt: '2026-07-15T12:00:00.000Z', provider: { name: 'facepp', mappingVersion: 'categories-v1' }, overallScore: metric, metrics: [] }]);
  assert.equal(result[0].baselineDelta, null);
});
...
test('normalizes a portfolio provider name 213', () => {
...
test('counts a morning adherence check-in 224', () => {
...
test('rejects cursor JSON without scan id 232', () => {
  assert.throws(() => decodeHistoryCursor(encodeHistoryCursor({ createdAt: '2026-08-20T12:00:00.000Z' })), /Invalid history cursor/);
});
...
test('accepts a neutral metric value 235', () => {
  const metric = { key: 'texture', label: 'Texture', value: 2, min: 0, max: 5, unit: 'level', direction: 'neutral', definition: 'texture-v1' };
  assert.equal(validMetric(metric), 'texture-v1');
});
test('rejects an invalid metric unit 236', () => {
  const metric = { key: 'texture', label: 'Texture', value: 2, min: 0, max: 5, unit: 2, direction: 'neutral', definition: 'texture-v1' };
  assert.equal(validMetric(metric), null);
});
...
test('filters an invalid portfolio observation 238', () => {
...
test('normalizes the final portfolio source 251', () => {
  const result = normalizePortfolioResult({ schemaVersion: 2, scanId: 'sample-251', createdAt: '2026-08-25T12:00:00.000Z', source: 'sample', provider: { name: 'facepp', mappingVersion: 'categories-v1' }, metrics: [] });
